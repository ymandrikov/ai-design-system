...
- Keep shared documents, indexes, journals and user questions with the main agent.
  Author dependent patterns only after required component contracts are ready;
  serialize overlapping edits and report newly discovered dependencies.
- Accept results from actual files and evidence, update shared documents and progress,
  and check the combined state. A worker's completion claim is insufficient.
- Preserve whole-set discovery timing and public-input isolation; use a separate
  fresh reviewer for final consistency. Correct actionable findings and obtain
  fresh review of affected final files before declaring completion.

**Independent variants:**

- Interrupt one author after partial writes. Continue unrelated work, pause its
  dependents, stop the previous writer before reassignment, inspect partial files
  and finish only the remaining work. Progress must reflect accepted results.
