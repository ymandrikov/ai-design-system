...
> New density policy and Unspecified recipe. Refresh their assessments and
...
- Update Routing summary to A with the deciding contract and a reproducible
  discovery verification path. Leave the incorrect index unchanged during triage.
- Assess Existing density decision as B and state one concrete question that
  recovers the existing decision without selecting a new policy.
- Assess New token role and New density policy as C with demonstrated required
  changes. One question sufficient to create a rule does not make it B.
- Assess Unspecified recipe as C because A/B are not proven, explicitly separating
  that uncertainty from a demonstrated need for a new pattern. Preserve its original
  expectation and retain the entry.
- Keep intervention separate from priority: the blocked token task precedes the
  inconvenient routing correction despite their C and A levels.
- Preserve all unrelated entries, including missing assessments. Do not repair,
  create rules/tokens, archive unresolved entries or weaken expectations.
