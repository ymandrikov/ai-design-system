...
Component, layout and pattern contracts use YAML frontmatter and an H1 name.
Use the section templates below as a starting point: omit inapplicable sections,
reorder sections and add explanatory sections where useful. Keep included sections
nonempty; H3 headings are optional. Preserve the information needed to select,
configure and compose the entity, including applicable behaviour and accessibility
obligations. Omission is appropriate only when the topic does not apply.
Group follows the standard directory and its index linked from DESIGN.md.
...
Only listed sources participate, excluding paths ending in `.css`, `.scss`, `.sass`,
`.less`, `.styl` or `.stylus` (case-insensitive, including CSS modules).
Excluded styles remain in `sources` and their paths must resolve; their contents and
membership do not affect the hash. A nonempty list containing only styles hashes the
empty JSON array. Tests/examples and unlisted imports are also excluded.
Other files are hashed in full, including inline styles, CSS-in-JS and style blocks
in components; formatting and comments count as changes. A missing or mismatched hash
requires [another contract review](contract.md#ordinary-audit). Matching hashes establish
unchanged included sources, not semantic correctness or runtime readiness. Style
changes still require applicable visual checks. Existing hashes that included style
files need a one-time update after review. With
...
The checker validates metadata, nonempty included sections, paths, links and source
hashes. It does not assess whether omitted topics are inapplicable or whether selection
logic and public-use information are sufficient; review those against the rules below.
...
Store the entire purpose in frontmatter `description`.
...
**When to use** makes the logic between conditions clear: which must hold together
and which are alternatives. Equivalent prose is sufficient; the literal words
`all` and `any` are optional.
...
Any listed disqualifier excludes the entity. If there are none, omit the section
or state that there are no disqualifiers.
...
**Public API** describes each supported binding, with a minimal valid example,
inputs/defaults/constraints, children or slots, events and methods where applicable.
Explain native-attribute forwarding and component-owned attributes. Include
...
**Accessibility** distinguishes what the component provides from consumer obligations,
naming owners of applicable semantics, labels, error relationships, keyboard/focus
and form state.

## Layout

**Public API** follows the component rules for its actual bindings/regions.
**Composition** distinguishes requirements, recommendations and applicable exceptions.
Requirements name who owns spacing, size, order, adaptation, grouping and any
observable/native behaviour.
Name allowable children and linked component contracts. Recommendations include when
an alternative is reasonable; Exceptions name exact conditions. Distinguish
...
**Composition** distinguishes requirements, recommendations and applicable exceptions.
Describe information grouping, permissible substitutions, nesting, actions and
adaptation. Include consumer
