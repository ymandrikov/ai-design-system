...
benefit and effort for opportunities. Proposed actions remain within improve's
authorised scope. Assessment is complete when
...
not restart a whole-system search after fixes. When verification is enabled, use the
single [final reconciliation](completion.md#reconcile-final-results), supplying the
selected improvements and craft/consumer check results. Account for every area and finding, separating
