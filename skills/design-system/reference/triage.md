...
separate request. Search the archive only for records relevant to the reviewed entries.
For an explicit archive cleanup request, follow [archive maintenance](archive-cleanup.md).
...
rules unchanged during triage, except for reference destinations that must change
during explicitly requested archive cleanup.
