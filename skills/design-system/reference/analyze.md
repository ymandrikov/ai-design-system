...
authorized [craft](craft.md) or [use](use.md) task. Apart from context recovery under
the shared model, analysis changes only its report and evidenced [gap records](gaps.md), leaving code, contracts, indexes and rules intact.
...
If required indexes are missing or unreadable, apply
[missing-context handling](model.md#missing-project-context) and continue once readiness
is established. If recovery needs unavailable sources or unresolved decisions, report
those inputs and pause affected analysis without creating a system or an analysis report.
A broken individual entry is a coverage/evidence limit: report it and continue independent analysis without treating the entry as an unmanaged alternative.
