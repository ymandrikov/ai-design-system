...
indexes are missing. Reuse prior surveys
...
Supply the [plan and completion criteria](migration-planning.md#supply-completion-criteria),
changed documents and saved check results to one
[final reconciliation](completion.md#reconcile-final-results) when verification is
enabled. Then apply the [final independent review](delegation.md#review-the-final-result)
only if explicitly requested or required by project policy and verification is enabled.
