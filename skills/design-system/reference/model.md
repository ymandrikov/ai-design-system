...
When required context is missing, inspect existing authoritative sources first.
Within the task's scope, restore artifacts whose content follows unambiguously from
those sources and established generation rules, then resume the affected procedure.
For example, regenerate missing UI indexes from existing contracts using the
[index generator](../assets/inventory.md). Preserve contract content, statuses and
existing project decisions; recovery does not authorise new rules or initial connection.
Honour explicit read-only or no-restoration instructions.

If sources are unavailable, incomplete or contradictory, name the missing facts or
conflicting decisions and ask only for what cannot be established from the project.
Do not invent rules, sources or contracts, or treat missing contract directories as
proof of an empty system. Pause only dependent work; continue independent authorised
work and resume once the missing input is resolved.

Standalone analysis retains its own [artifact-based readiness](analyze.md#check-readiness):
DESIGN.md is optional when its required indexes and artifacts are available.
...
checks, including separate final reconciliation and post-cleanup rechecks;
...
Free-form reports do not change the [contract requirements](formats.md),
