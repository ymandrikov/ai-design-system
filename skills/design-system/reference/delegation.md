...
by project policy, and verification is enabled. Otherwise finish with ordinary checks
and document reconciliation; reviewer availability is not a completion condition.
...
findings remain explicit blockers. If an explicitly required independent reviewer is unavailable, finish
