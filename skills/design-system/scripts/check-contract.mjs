...
const KINDS = ["component", "layout", "pattern"];

export function checkContract(markdown, { resolveLink = () => true, kind = "component" } = {}) {
  if (!KINDS.includes(kind)) return [`unknown contract kind: ${kind}`];
...
export function checkContractFile(path, { inventoryPath, inventoryPaths = [], kind = "component", updateSourcesHash = false } = {}) {
  ...
  if (problems.length === 0 && metadata.sources.length > 0) {
    ...
    const sources = [...new Set(metadata.sources)]
      .filter((path) => !/\.(css|scss|sass|less|styl|stylus)$/i.test(path))
      .sort().map((path) => [path, sha256(readFileSync(resolve(root, path)))]);
