...
You need Node.js with [pnpm](https://pnpm.io/installation), Git, and a coding agent that supports skills.
...
indexes under `design-system/`. Missing indexes can be regenerated from existing
contracts under the shared context-recovery rule; unresolved inputs pause affected analysis. Missing DESIGN.md alone does not block analysis. Empty index groups are valid.
...
Ask `design-system setup` to connect a new or existing project, continue an unfinished
connection, or complete missing parts of an existing connection. It preserves current
decisions and restores artifacts from authoritative sources without a completion marker.
Other procedures retain their own entrypoints; they do not reopen connection work.

Setup connects DESIGN.md to real sources, moves existing contracts to standard
locations with their content and statuses preserved, repairs affected references and
generates indexes. These moves do not require full migration. Root AGENTS.md gets the
main pointer; existing CLAUDE.md is preserved with a short pointer when needed.
Existing symlinks keep their shape and their shared source is edited once. Nested
instructions remain scoped. Agent-mode detection, instruction consolidation, new
symlinks and general skill compatibility audits are outside ordinary setup; request
reorganisation or an audit separately. Installed skills retain their actual locations.
See [setup](skills/design-system/reference/setup.md).

After preparing connection, the agent shows remaining undocumented reusable UI and
offers full automatic contracts-only migration or connection only. This covers the
whole project unless explicitly limited, preserving runtime code and appearance.
Migration starts only with explicit confirmation; existing permission is reused.
The same round collects the run-or-skip verification choice for connection and the
selected migration. Declining migration still allows connection checks.

The handoff to `migrate` supplies mode 3, scope and verification as settled inputs.
The agent chooses batches, saves progress and continues without repeated approvals.
Unresolved rules require decisions while independent work continues. Declining
migration leaves connection complete with uncovered UI reported.

For an empty project, DESIGN.md, instruction pointers, empty indexes and available
source links suffice; ask only about verification. Use the existing brief or library
and defer unknown design decisions to real tasks. No starter components or empty
journals are required. A request to connect and build a first screen covers both stages.

Gradual adoption and API analysis remain available by separate request through
[adoption](skills/design-system/reference/adoption.md); their existing batch choices
...
checks, evidence-gated admission and final reconciliation are skipped,
...
Checks run as changes become ready. One [final reconciliation](skills/design-system/reference/completion.md#reconcile-final-results)
compares requested outcomes, changed promises, documents and affected dependencies
with existing evidence. Only missing or invalidated checks run; unchanged instruction
files need no full reread. Independent assessment remains opt-in or project-required.
...
archive is created, and paused or blocked work keeps its execution materials. With
verification enabled, cleanup checks cover only transferred content, affected references
and any other checks invalidated by those changes.
...
Missing project context follows the [shared recovery rule](skills/design-system/reference/model.md#missing-project-context):
restore artifacts unambiguously derived from authoritative sources within scope,
preserving existing decisions. Ask about unavailable facts or conflicting decisions
and pause only dependent work. Explicit read-only limits still apply. Analyze remains available without root DESIGN.md when the required indexes
...
AGENTS.md points to root DESIGN.md, which connects sources and verification tools.
Existing CLAUDE.md files and symlinks are preserved, with a short pointer when needed.
The installed skill stays at its existing local or global path.
...
Purpose section lives in `description`. Section templates allow omitted inapplicable
sections, different ordering and additional explanations; selection logic and public-use
obligations must remain clear. The three
...
New gaps and entries reviewed during triage or repair retain their expected result,
evidence, next actionable step and any required decision.
[Gap recording](skills/design-system/reference/gaps.md#record-and-resolve) requires no
A/B/C classification; priority and repair authority remain separate.
The archive is retained regardless of size; search for relevant entries as needed.
[Archive cleanup](skills/design-system/reference/archive-cleanup.md) requires a separate
explicit request and preserves decision history, evidence and working links in project
documents. It requires neither Git history nor hosted permalinks.

### Source freshness

Contracts with nonempty `sources` require a `sourcesHash` frontmatter field. It records
the included source paths and file contents at the last review. Missing or different
hashes fail the check with exit code 1 and require another source-to-contract review.
Standalone styles are excluded according to the [hash format](skills/design-system/reference/formats.md#contract-frontmatter);
their paths are still validated and applicable visual checks remain required.
Inline styles, comments and formatting in other sources count as changes;
source-list ordering does not. Tests, previews and unlisted dependencies are excluded. `sources: []` skips the hash
...
Use the pnpm version pinned in `package.json`. Install the repository dependencies:

```sh
pnpm install --frozen-lockfile
```

Commit dependency changes with `pnpm-lock.yaml`. Dependency build scripts are limited
to esbuild in `pnpm-workspace.yaml`.
