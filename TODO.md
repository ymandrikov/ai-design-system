...
- [x] В начале миграции объяснять планируемые верификации и дожидаться выбора
  ...
  сущности сразу делать `discoverable`. Пропущенные и недоступные проверки не создают
  статусов, записей в контрактах или блокировок. Отдельная сверка документов при
  пропуске проверок не запускается; документы обновляются по ходу работы.
