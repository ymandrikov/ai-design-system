...
- From the enclosing workspace root (`../..`), `npm test -- <test-path>`
  runs focused Vitest tests; tests sit beside each binding. `npm test` runs all checks.
- Composition: `npm test -- fixtures/mini-ds/product/check-composition.test.tsx`.
...
- Preview: from `../..` run `npm run demo`, then `python3 -m http.server 4173`.
